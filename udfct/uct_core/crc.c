 ...
 * File        : crc.c
 *
 * Description : Source file used to calculate the CRC-CCITT checksum
 ...
 *               computeEAChecksum() copied from 6.8
 *
...
{
    while (n-- > 0)
        crc = (unsigned short) (crc_table[(crc>>8 ^ *s++) & 0xff] ^ (crc<<8));
    return crc;
}
...
extern unsigned short unicode_cksum(register unsigned short *s, register int n)
{
    ...
    while (n-- > 0) {
        ...
        crc = (unsigned short) (crc_table[(crc>>8 ^ (*s>>8)) & 0xff] ^ (crc<<8));
        crc = (unsigned short) (crc_table[(crc>>8 ^ (*s++ & 0xff)) & 0xff] ^ (crc<<8));
    }
    return crc;
}
